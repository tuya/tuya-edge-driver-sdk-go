...
func TestSecretRequest(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			require.NoError(t, err)
			req.Header.Set(contracts.CorrelationHeader, expectedCorrelationId)
...
func doRequest(t *testing.T, method string, api string, handler http.HandlerFunc, body io.Reader) *httptest.ResponseRecorder {
	...
	assert.Equal(t, contracts.ContentTypeJSON, recorder.HeaderMap.Get(contracts.ContentType), "Content type not set or not JSON")
	assert.Equal(t, expectedCorrelationId, recorder.HeaderMap.Get(contracts.CorrelationHeader), "CorrelationHeader not as expected")
