...
func init() {
	...
	resp, _ := dpc.AllDeviceProfiles(ctx, []string{}, 0, -1)
	for i := range resp.Profiles {
		dps = append(dps, dtos.ToDeviceProfileModel(resp.Profiles[i]))
	}
}
...
func TestProfileCache_Add(t *testing.T) {
	dpc := newProfileCache(dps)
	if err := dpc.Add(dtos.ToDeviceProfileModel(mock.NewDeviceProfile)); err != nil {
	...
	if err := dpc.Add(dtos.ToDeviceProfileModel(mock.DuplicateDeviceProfileRandomFloatGenerator)); err == nil {
...
func TestProfileCache_Remove(t *testing.T) {
	...
	if err := dpc.RemoveById(mock.NewDeviceProfile.Id); err == nil {
		t.Error("supposed to get an error when removing a device profile which doesn't exist in cache")
	}

	if err := dpc.RemoveById(mock.DeviceProfileRandomBoolGenerator.Id); err != nil {
...
func TestProfileCache_Update(t *testing.T) {
	...
	if err := dpc.Update(dtos.ToDeviceProfileModel(mock.NewDeviceProfile)); err == nil {
	...
	if err := dpc.Update(dtos.ToDeviceProfileModel(mock.DeviceProfileRandomBoolGenerator)); err != nil {
